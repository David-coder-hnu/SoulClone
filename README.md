...
**每一页都有氛围**：AmbientBackground 为每个场景生成独特的粒子光晕——聊天的青色迷雾、发现的洋红星云、主页的双色交织。12 个浮动粒子 + 双光晕 orb，Framer Motion 驱动，零 three.js 开销。

**每一次切换都是 dissolve**：页面转场不是"跳"，是灵魂溶解（blur + scale + opacity）后重新凝结。0.45 秒，ease `[0.16, 1, 0.3, 1]`。旧页面消散，新页面聚焦成形。

**导航是浮动的**：没有 sidebar chrome。一个底部居中的玻璃 Dock，hover 时 1.15x 放大、呼吸光效、active dot 弹簧动画。桌面和移动端统一。

**下线是一场仪式**：点击"下线"时，全屏意识交接动画——你的头像淡出模糊，渐变光束携白色粒子从左流到右，孪生头像从 blur 中聚焦成形。底部一行字：*"你关机的那一刻，另一个你，正在真诚地与世界说你好。"*
